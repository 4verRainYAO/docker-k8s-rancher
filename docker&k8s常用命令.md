...
## 1.docker用户过渡到kubectl命令行指南

针对docker-ui用户介绍Kubernetes 命令行如何与 api 进行交互。
...
默认情况下镜像会在后台运行，与`docker run -d ...` 类似，如果想在前台运行，使用：
...
pod跟容器之间的细微差别：默认情况下如果 pod 中的进程退出 pod 也不会终止，相反它将会重启该进程。这类似于 docker run 时的 `--restart=always` 选项， 这是主要差别。在 docker 中，进程的每个调用的输出都是被连接起来的，但是对于 kubernetes，每个调用都是分开的。要查看以前在 kubernetes 中执行的输出，请执行以下操作：
...
| `-o=jsonpath=             | 打印 [jsonpath](https://kubernetes.io/docs/user-guide/jsonpath) 表达式中定义的字段 |
| `-o=jsonpath-file=`       | 打印由 文件中的 [jsonpath](https://kubernetes.io/docs/user-guide/jsonpath) 表达式定义的字段 |
| `-o=name`                 | 仅打印资源名称                                               |
